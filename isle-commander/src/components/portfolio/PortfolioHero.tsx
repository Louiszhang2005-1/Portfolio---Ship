  ...
  return (
    <section id="about" className="h-screen w-full bg-[#050505] p-2 text-[#e1e0cc] sm:p-4">
      <div className="relative h-full w-full overflow-hidden rounded-2xl md:rounded-[2rem]">
        ...
        />
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_70%_28%,rgba(89,234,251,0.22),transparent_24%),linear-gradient(180deg,rgba(0,0,0,0.42),rgba(0,0,0,0.12)_38%,rgba(0,0,0,0.82))]" />
        <div
        ...
        <nav className="absolute left-1/2 top-0 z-20 -translate-x-1/2">
          <div className="flex max-w-[calc(100vw-1rem)] items-center gap-3 overflow-x-auto rounded-b-2xl bg-black px-4 py-2 shadow-2xl [scrollbar-width:none] sm:gap-6 md:gap-10 md:rounded-b-3xl md:px-8 lg:gap-12">
            {navItems.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="whitespace-nowrap text-[10px] text-[#e1e0cc]/75 transition-colors hover:text-[#e1e0cc] sm:text-xs md:text-sm"
              >
                {item.label}
              </a>
            ))}
        ...
        <div className="absolute bottom-0 left-0 right-0 px-4 pb-2 sm:px-6 md:px-10">
          <div className="grid grid-cols-12 items-end gap-4">
            <div className="col-span-12 lg:col-span-8">
              <h1 className="font-headline text-[22vw] font-medium leading-[0.85] tracking-[-0.07em] text-[#e1e0cc] sm:text-[20vw] md:text-[18vw] lg:text-[15vw] xl:text-[14vw]">
                <WordsPullUp text="Louis Zhang" showAsterisk />
              </h1>
            </div>
            ...
            <div className="col-span-12 flex flex-col gap-5 pb-6 lg:col-span-4 lg:pb-10">
              <motion.p
                ...
                className="max-w-md text-xs leading-[1.25] text-[#e1e0cc]/72 sm:text-sm md:text-base"
              >
