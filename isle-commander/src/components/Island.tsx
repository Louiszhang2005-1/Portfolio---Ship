...
export default function Island({ mission, isNearby, isVisited, index, onClick }: IslandProps) {
  ...
  const isTesla = mission.id === "I-1"; // Tesla Nevada — High Voltage sector

  // Track approach transitions to trigger the hop animation
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.94 }}
    >
      {/* ── Squash-and-Stretch island hop (triggers on each approach) ── */}
      <motion.div
        key={`hop-${hopCount}`}
        animate={hopCount > 0 ? {
      ...
      >
        <div className="relative flex flex-col items-center">
          ...
          {/* ── Tesla Nevada: High-Voltage construction barrier ── */}
          {isTesla && (
            <>
              {/* HV warning banner */}
              <div className="absolute -top-10 left-1/2 -translate-x-1/2 z-20 whitespace-nowrap">
                <div
                  className="px-3 py-1 rounded-full text-[9px] font-label font-bold uppercase tracking-widest shadow-lg border"
                  style={{
                    background: "rgba(15, 10, 0, 0.9)",
                    borderColor: "#ffee00",
                    color: "#ffee00",
                    boxShadow: "0 0 8px #ffee0066",
              ...
              {/* Lightning bolts at cardinal points */}
              {HV_BOLT_POSITIONS.map((pos, bi) => (
                <div
                  key={bi}
                  className="absolute z-20 select-none pointer-events-none text-lg"
                  style={{
                    ...pos,
                    animation: `hvZap 2s ${bi * 0.5}s infinite`,
                  }}
                >
                  ⚡
                </div>
          ...
          <div
            ...
            } ${isNearby && !isLocked ? "ring-4 ring-white/60 shadow-[0_0_40px_rgba(255,255,255,0.3)]" : ""}`}
            style={{
